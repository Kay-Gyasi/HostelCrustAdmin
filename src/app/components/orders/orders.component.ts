...
import { AddCompletedOrderAction, AddCompletedOrderSuccessAction, DeleteOrderAction, DeleteOrderSuccessAction, LoadOrdersAction } from 'src/app/state/store/action/hostel.action';
...
export class OrdersComponent implements OnInit {
  ...
  finishOrder(id:number, order:Order){
    ...
    this.store.dispatch(new DeleteOrderSuccessAction(order));

    this.store.dispatch(new AddCompletedOrderSuccessAction(order));
  };
}
